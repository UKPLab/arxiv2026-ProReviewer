...
class AgentSdkEngine:
    ...
    def transform_results_for_verl(self, episodes: list[Episode], task_ids: np.ndarray) -> "DataProto":
        ...
        for i, episode in enumerate(episodes):
            ...
            if all(len(trajectory.steps) == 0 or trajectory.info.get("judge_failed") for trajectory in episode.trajectories):
                # termination hits before an agent finishes it's first step,
                # or all trajectories are invalid (e.g., judge failed)
                # we delete the episode from the batch by setting repeat_counts to 0
                print(f"Episode {episode.id} has no valid trajectories (empty or judge_failed), dropping it from the batch")
                repeat_counts.append(0)
                continue
            ...
            for trajectory in episode.trajectories:
                ...
                if trajectory.info.get("judge_failed"):
                    logger.info(f"Trajectory {trajectory_id} judge failed, skipping")
                    continue
