...
def collect_reward_and_advantage_from_trajectory_groups(
    ...
    for group in groups:
        ...
        if algorithm_config.stepwise_advantage_mode == "broadcast":
        ...
        elif algorithm_config.stepwise_advantage_mode == "step_level":
            # Step-level GRPO: pool all steps from all trajectories, compute advantage across all
            all_steps = []
            all_step_rewards = []
            ...
            step_rewards_arr = np.array(all_step_rewards)
            rewards_by_group[group_role].extend(step_rewards_arr)

            if collect_advantage:
                advantages = advantage_fn(step_rewards_arr)
                advantages_by_group[group_role].extend(advantages)
                for step, advantage in zip(all_steps, advantages, strict=False):
                    step.advantage = advantage
