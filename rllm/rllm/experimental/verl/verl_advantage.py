...
def compute_advantage_verl(batch: DataProto, config: DictConfig) -> tuple[DataProto, dict]:
    ...
    metrics = {}
    if config.rllm.stepwise_advantage.mode == "step_level":
        # Step-level GRPO: group all steps by prompt (task_id)
    ...
    if config.rllm.stepwise_advantage.mode in ("per_step", "step_level"):
