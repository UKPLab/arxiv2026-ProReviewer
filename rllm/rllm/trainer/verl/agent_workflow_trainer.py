...
class AgentWorkflowPPOTrainer(RayPPOTrainer):
    ...
    def _save_checkpoint(self):
        """Override to retry checkpoint saving on NFS write failures."""
        import os
        max_retries = 3
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    print(
                        f"[checkpoint] Attempt {attempt}/{max_retries} failed: {e}\n"
                        f"  Cleaning up partial checkpoint at {local_global_step_folder} "
                    ...
                    print(f"[checkpoint] All {max_retries} attempts failed. Skipping checkpoint at step {self.global_steps}.")
                    if os.path.exists(local_global_step_folder):
                        shutil.rmtree(local_global_step_folder, ignore_errors=True)
                    raise
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        # Set uid for GRPO grouping based on stepwise advantage mode:
                        # - step_level: group all steps by task (paper), GRPO across all steps
                        # - broadcast: group by trajectory_id so all rollouts of the same task
                        #   form a GRPO group; the last-step advantage is then broadcast to all steps.
                        #   (step_ids are unique per step, so using them would create group-size-1
                        #   and GRPO normalization would be a no-op)
                        # - per_step / default: step_ids (== trajectory_ids when enable=False)
                        if self.config.rllm.stepwise_advantage.enable and self.config.rllm.stepwise_advantage.mode == "step_level":
                            batch.non_tensor_batch["uid"] = batch.non_tensor_batch["task_ids"]
                        elif self.config.rllm.stepwise_advantage.enable and self.config.rllm.stepwise_advantage.mode == "broadcast":
                        ...
                        if self.config.rllm.stepwise_advantage.enable and self.config.rllm.stepwise_advantage.mode in ("per_step", "step_level"):
                        ...
                        if self.distill_enabled:
                            ...
                            if self.config.rllm.stepwise_advantage.enable and self.config.rllm.stepwise_advantage.mode == "broadcast":
                                ...
                                # Log GRPO grouping sanity check
                                uids = batch.non_tensor_batch["uid"]
                                unique_uids = np.unique(uids)
                                mask = batch.batch["response_mask"]
                                advs = batch.batch["advantages"]
                                scalar_advs = [(advs[i][mask[i].bool()].mean().item() if mask[i].any() else 0.0) for i in range(len(uids))]
                                print(f"[broadcast-adv] {len(uids)} last steps, {len(unique_uids)} GRPO groups, "
                                      f"adv range=[{min(scalar_advs):.4f}, {max(scalar_advs):.4f}], "
                                      f"adv std={np.std(scalar_advs):.4f}")
    ...
    def visualize_trajectory_last_step(self, tensor_batch, sample_idx=0, max_samples=1):
        ...
        visualize_trajectories(
            ...
            reward_key="step_rewards" if self.config.rllm.stepwise_advantage.enable and self.config.rllm.stepwise_advantage.mode in ("per_step", "step_level") else "traj_rewards",
