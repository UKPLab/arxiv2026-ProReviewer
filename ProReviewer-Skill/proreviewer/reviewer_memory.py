...
# ---------------------------------------------------------------------------
# Conference rating-scale definitions
#
# Sources (fetched 2026-07-02):
#   ICML 2026:    https://icml.cc/Conferences/2026/ReviewerInstructions
#   NeurIPS 2026: https://neurips.cc/Conferences/2026/MainTrackHandbook
#   ACL / EMNLP:  https://aclrollingreview.org/reviewform  (ARR, Feb 2025+)
#   ICLR 2026:    {0,2,4,6,8,10} per https://arxiv.org/pdf/2511.15462
#                 (label text adapted from the prior {1,3,5,6,8,10} scale;
#                  exact 2026 wording is only on the OpenReview form itself)
# ---------------------------------------------------------------------------
...
def get_valid_scores(conference: str) -> List:
    """Return the list of valid score values for a conference."""
    key = conference.lower()
    if key not in CONFERENCE_SCALES:
        raise ValueError(
            f"Unknown conference '{conference}'. "
            f"Supported: {', '.join(CONFERENCE_SCALES.keys())}"
        )
    return CONFERENCE_SCALES[key]["scores"]


def validate_score(score, conference: str) -> None:
    """Raise ValueError if score is not valid for the conference scale."""
    valid = get_valid_scores(conference)
    if score not in valid:
        scale = CONFERENCE_SCALES[conference.lower()]
        raise ValueError(
            f"Score {score} is not valid for {scale['name']}. "
            f"Valid scores: {valid}"
        )


def format_score_with_scale(score, conference: str) -> str:
    """Format a score with its conference scale context."""
    key = conference.lower()
    scale = CONFERENCE_SCALES.get(key)
    if not scale:
        return str(score)
    label = scale["labels"].get(score, "")
    name = scale["name"]
    hi = max(scale["scores"])
    if label:
        return f"{score}/{hi} ({name}: {label})"
    return f"{score}/{hi} ({name})"
...
class ReviewOutline(BaseModel):
    ...
    overall_score: Optional[Union[int, float]] = Field(default=None, description="Single overall score (scale depends on conference)")
    conference: str = Field(default=DEFAULT_CONFERENCE, description="Conference whose rating scale to use")
    ...
    def to_prompt_str(self, brief: bool = True, show_evidence_tags: bool = True) -> str:
        ...
        if self.overall_score is not None:
            parts.append(f"Overall Score: {format_score_with_scale(self.overall_score, self.conference)}")
...
class ReviewLog(BaseModel):
    ...
    conference: str = Field(
        ...
        description="Conference whose rating scale to use (iclr, icml, neurips, acl, emnlp)"
    )
    ...
    def update_outline(
        ...
        if section == "summary":
        ...
        elif section in ("strengths", "weaknesses", "questions"):
            ...
            items_list = getattr(self.review_outline, section)
            dup_label = {"strengths": "strength", "weaknesses": "weakness", "questions": "question"}[section]
            if append:
                if self._check_duplicate(item.text, dup_label):
                    return "duplicate_skipped"
                items_list.append(item)
            else:
                setattr(self.review_outline, section, [item])

        elif section == "overall_score":
            if content is not None:
                score = float(content)
                # Use int if the value is whole (e.g. 4.0 -> 4)
                if score == int(score):
                    score = int(score)
                validate_score(score, self.conference)
                self.review_outline.overall_score = score
            else:
                self.review_outline.overall_score = None
            self.review_outline.conference = self.conference
