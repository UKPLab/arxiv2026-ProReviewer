"""
Review Utility Inference Prompts

This module contains the prompts used for inference in the review utility system.
Extracted from prompt.py and utils.py for clean separation of inference logic.
"""

# System prompt header used for all evaluations
...
# System prompt header when paper text context is provided
...
# User prompt tails for different generation modes
SCORE_ONLY_PROMPT_TAIL = '''
Evaluate the review based on the given definitions of the aspect(s) above. Output only the numeric score for each aspect in JSON format.
...
# Aspect definitions (without examples) used during inference
ASPECTS_NO_EXAMPLES = {
"actionability":
'''
**Actionability**
...
def build_inference_prompt(review_point, aspects='all', include_paper_text=False, paper_text=None,
                          generation_type='score_and_rationale'):
    """
    Build an inference prompt for evaluating review comments.

    Args:
        review_point (str): The review comment to evaluate
        aspects (str or list): Aspect(s) to evaluate. Options: 'all', 'actionability',
                              'grounding_specificity', 'verifiability', 'helpfulness'
                              or a list of aspect names
        include_paper_text (bool): Whether to include paper context in the prompt
        paper_text (str): The paper text (required if include_paper_text=True)
        generation_type (str): 'score_only' or 'score_and_rationale'

    Returns:
        list: Chat-formatted prompt with 'role' and 'content' keys
    """
    # Determine which aspects to include
    all_aspects = ['actionability', 'grounding_specificity', 'verifiability', 'helpfulness']

    if isinstance(aspects, str):
        if aspects == 'all':
            considered_aspects = all_aspects
        else:
    ...
    if include_paper_text and paper_text:
