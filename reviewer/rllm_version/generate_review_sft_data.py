...
from reviewer.reward.calculator import RewardCalculator, split_review_text
from reviewer.reward.prompts import DEFAULT_JUDGE_MODEL
...
async def compute_human_review_utility(
    ...
    scores each point using _evaluate_single_point_async(), and returns the average.
    ...
    scores = []
    for point in points:
        score, _ = await calculator._evaluate_single_point_async(point)
        scores.append(score)
...
async def process_paper(
    ...
    for review in qualified_reviews:
        ...
        if calculator is not None and utility_threshold is not None:
            utility_score, num_points = await compute_human_review_utility(calculator, review)
...
async def async_main(args):
    ...
    if args.utility_threshold is not None:
        ...
        calculator = RewardCalculator(judge_model=judge_model, api_key=args.judge_api_key)
        logger.info(
    ...
    tasks = [
        process_paper(
            ...
            args.skip_existing, calculator, args.utility_threshold
        )
